...
function tie_clean_options(&$value) {
  $value = htmlspecialchars(stripslashes( str_replace( 'tie-open-tag', '', $value ) ));
}
					...
					<div class="option-item" id="breaking_cat-item">
						...
						<br /><small><?php _e( 'Hold CTRL while selecting to select multiple categories.', 'tie' ) ?></small>
							...
							<?php
								...
								if( empty( $breaking_cat ) || ! is_array( $breaking_cat ) ){
									$breaking_cat = array();
								}

								foreach ( $categories as $key => $option ) { ?>
									<option value="<?php echo $key ?>" <?php if ( in_array( $key , $breaking_cat ) ) { echo ' selected="selected"' ; } ?>><?php echo $option; ?></option>
								<?php
								}
							?>
						</select>
