jQuery(window).on( 'load', function() {
	...
	if (selected_item == 'template-blog.php' || selected_item == 'template-masonry.php' || selected_item == 'template-media.php' ) {
		jQuery('#tie-template-blog').show();
	}
	else if (selected_item == 'template-best-reviews.php') {
		jQuery('#tie-template-blog, #tie_posts_num-item').show();
	}
	else if (selected_item == 'template-authors.php') {
		jQuery('#tie-template-authors').show();
	}
...
jQuery(document).ready(function() {
	...
	jQuery('.tie-tooltip').each(function() {
		jQuery(this).tooltip({
			content: function() {
				return jQuery(this).prop('title');
			},
			tooltipClass: 'tie-ui-tooltip',
			position: {
				my: 'center top',
				at: 'center bottom+10',
				collision: 'flipfit'
